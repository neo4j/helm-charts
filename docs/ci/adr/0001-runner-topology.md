...
- GKE capacity stays available for the cases that actually need it: merges to `5.26`, scheduled auto-releases, and manual dispatches.
...
- Gating rules: eventually `pr-fast` should be a required check on `5.26`, with the full suite remaining required before release. For now, `pr-fast` is advisory.
