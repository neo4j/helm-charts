...
func k8sTests(name *model.ReleaseName) []SubTest {
	return []SubTest{
		{name: "Check Neo4j Configuration", test: func(t *testing.T) {
			assert.NoError(t, CheckNeo4jConfiguration(t, name, neo4jConfFile), "Neo4j Config check should succeed")
		}},
		...
		{name: "Reinstall Resources", test: func(t *testing.T) {
			assert.NoError(t, ResourcesReinstall(t, name), "Reinstall Resources should succeed")
		}},
		...
		{name: "Check Service Annotations", test: func(t *testing.T) {
			assert.NoError(t, CheckServiceAnnotations(t, name), "Services should have annotations")
		}},
